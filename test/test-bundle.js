...
ExpressionNode = (function(superClass) {
  ...
  ExpressionNode.prototype.destroyInstance = function(index) {
    return this.removeChild(this.orderedChildrenKeys[index]);
  };
          ...
          case 'kleene':
            switch (piece.type) {
              ...
              case 'input':
                return _this.addChild("" + piece.identifier, wrapInExpression(piece, piece.quantifier));
              case 'literal':
                return _this.addChild("@" + (literalCounter++), wrapInExpression(piece, piece.quantifier));
            }
        }
      };
    })(this));
...
MockInput = (function(superClass) {
  ...
  function MockInput(identifier, quantifier, acceptCondition) {
    this.identifier = identifier;
    this.quantifier = quantifier;
    this.acceptCondition = acceptCondition;
...
describe('Quantifiers', function() {
  ...
  it('generally work', function() {
    ...
    expect(inst1.expressions['abyss_option'].instances.length).toBe(1);
    expect(inst1.expressions['abyss_option'].instances[0]).toBe(optionHole);
    expect(optionHole.holes['abyss_option']).toBeDefined();
    expect(optionHole.holes['abyss_option'].isFilled).toBe(false);
    optionHole.holes['abyss_option'].fill(new MockTemplate(this.one, 'will-be-replaced'));
    expect(optionHole.holes['abyss_option'].isFilled).toBe(true);
    expect(optionHole.holes['abyss_option'].expression.template).toBe(this.one.pieces);
    previousFill = optionHole.holes['abyss_option'].expression;
    optionHole.holes['abyss_option'].fill(new MockTemplate(this.flag, 'replacement'));
    expect(optionHole.holes['abyss_option'].isFilled).toBe(true);
    expect(optionHole.holes['abyss_option'].expression.template).toBe(this.flag.pieces);
    expect(optionHole.childList.length).toBe(1);
    expect(optionHole.holes['abyss_option'].expression).not.toBe(previousFill);
    inst2 = this.root.expression.instantiate();
    expect(this.root.expression.childList.length).toBe(1);
