...
var App, Grammar, Hole, Literal, Node, Regex, Sequence, SyntaxTree, Variadic, _, match, parseGrammar, ref, ref1, ref2;
...
ref1 = require('Grammar'), Grammar = ref1.Grammar, Sequence = ref1.Sequence, (ref2 = ref1.S, Literal = ref2.Literal, Hole = ref2.Hole, Regex = ref2.Regex, Variadic = ref2.Variadic);
...
App = (function() {
  ...
  App.prototype.setup = function() {
    ...
    onFlowerPickerSelect = (function(_this) {
      return function(event) {
        var model, ref3, sequence;
        model = event.detail.value;
        sequence = _this.syntaxTree.grammar.makeSequence(model.group, model.identifier, model.data);
        return (ref3 = _this._activeHole) != null ? ref3.nodeModel.fill(sequence) : void 0;
      };
    })(this);
  ...
  App.prototype.loadState = function(syntaxTree) {
    ...
    syntaxTreeToTextTree = function(st) {
      var helper;
      helper = function(node, holeId, holeIndex) {
        if (node.isFilled) {
          return {
            type: 'branch',
    ...
    updateView();
    return this.syntaxTree.addEventListener('changed', updateView);
  };
  ...
  App.prototype.setActiveHole = function(path, useNumericPath) {
    ...
    if (nodeModel != null) {
      this._activeHole = {
        path: path,
        nodeModel: nodeModel,
        nodeElement: this._textRoot.navigate(path, useNumericPath).querySelector('.node')
      };
          ...
          custom = needsInput ? {
            ...
            display: function(model, data) {
              if ((data != null) && data.length > 0) {
                return (grammar.makeSequence(group, innerKey, data)).display();
              } else {
                return (grammar.makeSequence(group, innerKey)).display();
              }
            }
          } : {
...
window.addEventListener('WebComponentsReady', function() {
  ...
  mock.rules = _.mapValues(litRules, function(vo) {
    return _.mapValues(vo, function(vi) {
      ...
      return new Sequence(parsedSeq.map(function(arg) {
        var id, type, value;
        type = arg.type, value = arg.value, id = arg.id;
        switch (type) {
          case 'literal':
            return new Literal(value);
          case 'hole':
            return new Hole(value, id);
          case 'varargs':
            return new Variadic(value, id);
          case 'regex':
            return new Regex(value, id);
          default:
            return console.log('unrecognized production: ', type, value);
        }
      }));
    });
  });
  ...
  return Polymer.Gestures.add(document.querySelector('#render'), 'up', function(evt) {
    return console.log(app.syntaxTree.root.render());
  });
});


},{"Grammar":3,"Syntax":4,"lodash":2,"parsers/grammar-shorthand":5,"util/match":6}],2:[function(require,module,exports){
(function (global){
/**
 * @license
...
var Grammar, Hole, Literal, Regex, Sequence, Symbol, Variadic, _, match,
...
Grammar = (function() {
  ...
  Grammar.prototype.makeSequence = function(groupId, sequenceId, data) {
    var ref, ref1;
    if (data != null) {
      return (ref = this.productions[groupId]) != null ? ref[sequenceId].withData(data) : void 0;
    } else {
      return (ref1 = this.productions[groupId]) != null ? ref1[sequenceId] : void 0;
    }
  };
...
Sequence = (function() {
  function Sequence(symbols) {
    var holeIndex, sequenceIndex;
    this.symbols = symbols;
    this.holes = {};
    sequenceIndex = 0;
    holeIndex = 0;
    _.reduce(this.symbols, function(acc, elm) {
      return match(elm, {
    ...
    return this.symbols.map(function(sym) {
      return match(sym, {
        ...
        Regex: function(arg) {
          ...
          identifier = arg.identifier, pattern = arg.pattern;
          return "`" + identifier + "`";
        }
      });
    }).join('');
  ...
  Sequence.prototype.withData = function(data) {
    if (!(_.any(this.symbols, function(sym) {
      return sym.constructor.name === 'Regex';
    }))) {
      return this;
    } else {
      return new Sequence(this.symbols.map(function(sym) {
        if (sym.constructor.name === 'Regex') {
...
Variadic = (function(superClass) {
  extend(Variadic, superClass);

  function Variadic() {
    return Variadic.__super__.constructor.apply(this, arguments);
  }

  Variadic.prototype.display = function() {
    return "<" + this.identifier + "...>";
  };
...
Regex = (function(superClass) {
  ...
  function Regex(pattern1, identifier1) {
    this.pattern = pattern1;
    this.identifier = identifier1;
  }

  Regex.prototype.identifier = void 0;
...
var Grammar, Hole, Literal, Node, Regex, S, Sequence, SyntaxTree, Variadic, _, match, ref,
...
ref = require('Grammar'), Grammar = ref.Grammar, Sequence = ref.Sequence, S = ref.S;

Literal = S.Literal, Hole = S.Hole, Regex = S.Regex, Variadic = S.Variadic;
...
SyntaxTree = (function() {
  function SyntaxTree(grammar, startGroup) {
    ...
    this._listeners = {};
    this.root = Node.makeRoot(new Sequence([new Hole(startGroup, 'start')]));
  ...
  SyntaxTree.prototype.addEventListener = function(kind, callback) {
    var l;
    if (this._listeners[kind] == null) {
      this._listeners[kind] = [];
    }
    l = this._listeners[kind].push(callback);
    return (function(_this) {
      return function() {
        return _this._listeners[kind][l] = void 0;
      };
    })(this);
  };

  SyntaxTree.prototype.dispatchEvent = function(kind, details) {
    return this._listeners[kind].forEach(function(cb) {
      return typeof cb === "function" ? cb(details) : void 0;
    });
  };
...
Node = (function() {
   ...
   */
  function Node(parent, holeId, sequence) {
    this.dispatchEvent = bind(this.dispatchEvent, this);
    ...
    this._eventParent = this.parent = parent;
    if (sequence != null) {
  ...
  Node.makeRoot = function(sequence) {
    var result;
    result = new Node();
  ...
  Node.prototype.holeInformation = null;

  Node.prototype.fill = function(sequence) {
    this._setSequence(sequence);
  ...
  Node.prototype.walk = function(path, options) {
    var hd, nextChild, ref1, tl;
    if (options == null) {
      options = {};
    }
    if (path.length === 0) {
      if (options.endFn != null) {
        return options.endFn(this);
      } else {
        return this;
      }
    } else {
      ...
      nextChild = (function(_this) {
        return function() {
          if (options.useNumericPath) {
            return _this._getNthChild(hd);
          } else {
            return _this._getChild(hd);
          }
        };
      })(this)();
      if (nextChild == null) {
      ...
      if (((ref1 = options.fold) != null ? ref1.proc : void 0) != null) {
    ...
    return this.sequence.symbols.map((function(_this) {
      return function(symbol) {
        return match(symbol, {
          ...
          Variadic: function(arg) {
            var child, group, i, identifier, len, ref1, results;
            identifier = arg.identifier, group = arg.group;
            ref1 = _this.childrenMap[identifier];
            results = [];
          ...
          Hole: function(arg) {
            var group, identifier;
            identifier = arg.identifier, group = arg.group;
            if (_this.childrenMap[identifier].isFilled) {
              return (function() {
                return _this.childrenMap[identifier].render();
              })();
            } else {
              return '`' + group + '`';
            }
  ...
  Node.prototype.addEventListener = function(kind, callback) {
    var l;
    if (this._eventListeners[kind] == null) {
      this._eventListeners[kind] = [];
    }
    l = this._eventListeners[kind].push(callback);
    return function() {
      return this._eventListeners[kind][l] = void 0;
    };
  };

  Node.prototype.dispatchEvent = function(kind, details) {
    var ref1;
    if ((ref1 = this._eventListeners[kind]) != null) {
      ref1.forEach(function(cb) {
        return typeof cb === "function" ? cb(details) : void 0;
      });
    }
    return this._eventParent.dispatchEvent(kind, details);
  };

  Node.prototype.routeEvents = function(parent) {
    return this._eventParent = parent;
  };

  Node.prototype._getChild = function(id) {
    return this.childrenMap[id];
  };

  Node.prototype._getNthChild = function(n) {
    return this.children[n];
  };

  Node.prototype._setSequence = function(sequence) {
    var childCount, holeIndex, variadicChildrenMap;
    if (sequence == null) {
      return;
    }
    this.isFilled = true;
    ...
    this.sequence.symbols.forEach((function(_this) {
      return function(sym, index) {
        return match(sym, {
          Hole: function(hole) {
            return _this._holes[hole.identifier] = {
              id: hole.identifier,
              group: hole.group,
              isVariadic: false,
              sequenceIndex: index,
          ...
          Variadic: function(varargs) {
            return _this._holes[varargs.identifier] = {
    ...
    this.children = [];
    variadicChildrenMap = {};
    this.childrenMap = _.mapValues(this._holes, (function(_this) {
      return function(v, k) {
        var child, scope, variadicId;
        if (v.isVariadic) {
          variadicId = function(arg) {
            var __cloneNumber, holeInformation;
            __cloneNumber = arg.__cloneNumber, holeInformation = arg.holeInformation;
            return holeInformation.id + "-" + __cloneNumber;
          };
          child = new Node(_this, v.id);
          child.__cloneNumber = 0;
          child.variadicId = variadicId(child);
          _this.children[childCount++] = child;
          variadicChildrenMap[child.variadicId] = child;
          scope = _this;
          child.fill = function() {
            var sibling;
            Node.prototype.fill.apply(this, arguments);
            sibling = new Node(scope, this.holeInformation.id);
            sibling.fill = this.fill;
            sibling.__cloneNumber = this.__cloneNumber + 1;
            sibling.variadicId = variadicId(sibling);
            scope.children[childCount++] = sibling;
            scope.childrenMap[sibling.variadicId] = sibling;
            scope.childrenMap[this.holeInformation.id].push(sibling);
            return this.dispatchEvent('changed');
          };
          return [child];
        } else {
    ...
    return _.extend(this.childrenMap, variadicChildrenMap);
  };
...
module.exports = (function() {
  ...
  function parse(input) {
    var options = arguments.length > 1 ? arguments[1] : {},
        ...
        peg$c4 = function(text) { return wrap('literal', concat(text)) },
        peg$c5 = "`",
        ...
        peg$c15 = function(id, group) { return wrap('varargs', concat(group), concat(id)) },
        peg$c16 = function(id, group) { return wrap('hole', concat(group), concat(id)) },
        peg$c17 = ":\\",
        peg$c18 = { type: "literal", value: ":\\", description: "\":\\\\\"" },
        peg$c19 = function(id, pattern) { return wrap('regex', concat(pattern), concat(id)) },
...
module.exports = match = function(obj, fns) {
  ...
  } else if (fns["else"] != null) {
    return fns["else"].apply(null, arguments);
  } else {
